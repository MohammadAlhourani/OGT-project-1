...
void Client::SendPlayerData(const PlayerData& data)
{
	...
	m_pm.Append(gameU.toPacket(PacketType::PlayerData));
}

void Client::SetPlayer(Player* t_player)
{
	m_player = t_player;
}

void Client::setEnemies(Enemy& t_enemy1, Enemy& t_enemy2)
{
	m_enemy1 = &t_enemy1;
...
bool Client::GetPlayerData(PlayerData& data)
{
	...
	return recvall((char*)&data, bufferlength);
}

bool Client::GetGameState(GameStateUpdate& state)
{
	std::int32_t bufferlength; //Holds length of the message
	...
	return recvall((char*)&state, bufferlength);
}

bool Client::GetGameEndMsg(GameEnd& gEnd)
{
	std::int32_t bufferlength; //Holds length of the message
	...
	return recvall((char*)&gEnd, bufferlength);
}
